  ...
  const desktopLinks = useMemo(
    () => (
      <Box sx={{ display: "flex", gap: 2 }}>
        {!isMobile && isLoggedIn && (
          <Button startIcon={<GroupAddIcon />} color="inherit" component={Link} to="/app/createGroup">
            Create Group
          </Button>
        )}
        {!isLoggedIn && (
          <>
            <Button startIcon={<LoginIcon />} color="inherit" component={Link} to="/auth/login">
              Login
            </Button>
          ...
          {isLoggedIn && (
            <Box marginLeft="auto">
              <IconButton size="large" onClick={handleMenuOpen} color="inherit">
                <Avatar
                  alt={user?.name || "User"}
                  src={user?.imageUrl || `https://api.dicebear.com/5.x/initials/svg?seed=${user?.name}`}
